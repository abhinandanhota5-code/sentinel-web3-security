  ...
  return (
    <div className="mb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full liquid-pill text-[10px] font-mono tracking-wider uppercase text-emerald-300 mb-1">
        ...
        <div className="flex items-center liquid-glass-subtle rounded-xl p-1 self-start">
          ...
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-mono transition flex items-center gap-1.5 ${
              activeTab === 'history'
                ? 'bg-stone-500/20 text-stone-200 font-semibold border border-stone-500/40'
                : 'text-stone-400 hover:text-stone-200'
            }`}
          >
      ...
      <div className={`grid gap-6 ${activeTab === 'both' ? 'grid-cols-1 lg:grid-cols-2' : 'grid-cols-1'}`}>
        ...
        {(activeTab === 'both' || activeTab === 'exposure') && (
          <div className="liquid-glass rounded-2xl p-5 shadow-2xl relative overflow-hidden border-t border-t-rose-300/40">
            ...
            <div className="flex items-center justify-between pb-3.5 mb-4 border-b border-white/10">
              ...
              <span className="text-xs font-mono px-2 py-0.5 rounded-lg bg-rose-500/15 border border-rose-400/30 text-rose-200">
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      {item.blastRadiusUsd > 0 && (
                        <span className="text-xs font-mono font-bold text-rose-300 bg-rose-500/15 px-2 py-0.5 rounded-md border border-rose-400/30">
                          ${item.blastRadiusUsd.toLocaleString()} exposed
                        </span>
                      )}
                    </div>

                    <h4 className="text-xs font-bold text-white group-hover:text-amber-200 transition mb-1">
                      {item.title}
                    </h4>
                    ...
                    <div className="bg-black/30 border border-white/5 rounded-lg p-2 text-[10px] font-mono space-y-1">
                      <div className="flex items-center justify-between text-stone-400">
                        <span>Spender:</span>
                        <span className="text-stone-200">{item.counterparty.label || item.counterparty.address}</span>
                      </div>
                      <div className="flex items-center justify-between text-stone-400">
                        <span>State Proof:</span>
                        <span className="text-emerald-300 truncate max-w-[200px]" title={item.directEvidenceProof}>
                          {item.directEvidenceProof}
                        </span>
                      </div>
                    </div>

                    <div className="mt-2 pt-2 border-t border-white/5 flex items-center justify-between text-[10px] text-stone-400 font-mono">
                      <span>Since: {item.activeSince}</span>
                      <span className="text-amber-200 group-hover:translate-x-1 transition flex items-center gap-1 font-medium">
        ...
        {(activeTab === 'both' || activeTab === 'history') && (
          <div className="liquid-glass rounded-2xl p-5 shadow-2xl relative overflow-hidden border-t border-t-stone-300/40">
            
            <div className="flex items-center justify-between pb-3.5 mb-4 border-b border-white/10">
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-xl bg-stone-500/15 border border-stone-400/30 flex items-center justify-center text-stone-300">
              ...
              <span className="text-xs font-mono px-2 py-0.5 rounded-lg liquid-glass-subtle text-stone-300">
              ...
              {historicalActivities.map((act) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 text-[9px] font-bold rounded-md bg-white/10 text-stone-200 border border-white/10 uppercase">
                        {act.actionType}
                      </span>
                  ...
                  <p className="text-stone-200 font-medium text-[11px] mb-2 font-sans">
                    {act.description}
                  </p>

                  <div className="bg-black/25 border border-white/5 rounded-lg p-2 space-y-1 text-[10px] text-stone-400">
                    <div className="flex items-center justify-between">
                      <span>Tx:</span>
                      <span className="text-purple-300 truncate max-w-[180px]" title={act.transactionHash}>
                    ...
                    {act.valueUsd > 0 && (
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-stone-200 font-bold">${act.valueUsd.toLocaleString()}</span>
                      </div>
                    )}
                  </div>

                  <div className="mt-2 pt-1.5 text-[10px] text-stone-400 font-sans border-t border-white/5">
                    <span className="text-stone-300 font-semibold font-mono">Analysis: </span>
                    {act.note}
                  </div>
