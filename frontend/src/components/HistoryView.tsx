...
export const HistoryView: React.FC<HistoryViewProps> = ({
  ...
  const getActionBadge = (actionType: string) => {
    switch (actionType) {
      ...
      case 'SWAP':
        return 'bg-[#7dd3fc]/20 text-[#bae6fd] border-[#7dd3fc]/40';
      case 'TRANSFER':
  ...
  return (
    <section className="liquid-glass rounded-3xl p-5 sm:p-6 mb-6 shadow-2xl relative overflow-hidden border border-white/20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 mb-5 border-b border-white/10">
        <div>
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 rounded-full liquid-pill text-[10px] font-mono tracking-wider uppercase text-[#bae6fd] mb-1 border border-[#bae6fd]/30 bg-[#bae6fd]/10">
            <History className="w-3 h-3 text-[#bae6fd]" />
        ...
        <div className="text-xs font-mono text-slate-400">
          <span className="text-[#bae6fd] font-bold">Epistemic Rule:</span> Historical transactions do not drain funds today without lingering active state.
        </div>
      </div>
      ...
      {activities.length === 0 ? (
        <div className="liquid-glass-subtle rounded-2xl p-8 text-center border border-white/10">
          ...
          <h3 className="text-sm font-bold text-[#fdfbf7] font-mono">No historical anomalies recorded</h3>
        ...
        <div className="space-y-4">
          {activities.map((activity, idx) => {
            ...
            return (
              <div
                key={activity.id || idx}
                className={`liquid-glass-subtle rounded-2xl p-5 border transition flex flex-col justify-between shadow-lg backdrop-blur-xl relative overflow-hidden ${
                  isSuspicious ? 'border-amber-400/30' : 'border-white/10'
                }`}
              >
                {/* Header Row */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 mb-3 border-b border-white/10">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`px-2.5 py-0.5 rounded-md text-[10px] font-mono font-bold tracking-wider uppercase border ${getActionBadge(activity.actionType)}`}>
                      {activity.actionType.replace('_', ' ')}
                    </span>

                    {isSuspicious && (
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-mono font-bold uppercase bg-rose-500/20 text-rose-300 border border-rose-400/30 flex items-center gap-1">
                ...
                {/* Event Details Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs font-mono bg-black/30 rounded-xl p-3.5 border border-white/10 mb-3">
                  
                  {/* Transaction Hash */}
                  <div>
                    <span className="text-slate-400 block text-[10px] uppercase">Transaction Hash:</span>
                    <div className="flex items-center gap-1 mt-0.5">
                      <span className="text-[#bae6fd] truncate">
                      ...
                      <button
                        ...
                        title="Copy Tx Hash"
                      >
                        {copiedId === `tx-${idx}` ? <Check className="w-3 h-3 text-[#2dd4bf]" /> : <Copy className="w-3 h-3" />}
                      </button>
                      <a
                  ...
                  <div>
                    ...
                    <span className="text-[#fdfbf7] font-bold block mt-0.5">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      {activity.fromAddress && (
                        <button
                          ...
                          className="p-1 hover:text-white"
                        >
                          {copiedId === `from-${idx}` ? <Check className="w-3 h-3 text-[#2dd4bf]" /> : <Copy className="w-3 h-3" />}
                        </button>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      {(activity.toAddress || activity.counterparty?.address) && (
                        <button
                          ...
                          className="p-1 hover:text-white"
                        >
                          {copiedId === `to-${idx}` ? <Check className="w-3 h-3 text-[#2dd4bf]" /> : <Copy className="w-3 h-3" />}
                        </button>
                      )}
                    </div>
                  </div>
                        ...
                        <div className="space-y-1">
                          {activity.tokenTransfers.map((tt, tIdx) => (
                            <div key={tIdx} className="flex items-center justify-between text-[11px]">
                              <span className="text-[#bae6fd] font-bold">{tt.amount} {tt.symbol || 'Tokens'}</span>
                        ...
                        <div className="space-y-1 text-[11px]">
                          {activity.contractInteraction.method && (
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-[#93c5fd] font-bold">{activity.contractInteraction.method}</span>
                            </div>
                          )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
                  ...
                  {onSelectEvidence && (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-[#bae6fd]/20 border border-white/15 hover:border-[#bae6fd]/40 text-[#bae6fd] text-xs font-mono font-semibold transition flex items-center gap-1.5 self-end sm:self-auto cursor-pointer shrink-0"
                    >
