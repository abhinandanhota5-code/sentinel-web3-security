...
export const SYSTEM_PROMPT = [
  ...
  "3. OBSERVED facts: state plainly. EXTERNAL facts: cite the source and disclaimer.",
  "   INFERRED facts: label with \"Inferred:\" and state the basis. Never present an",
  "   inference or external report as a direct observation.",
...
function renderUnknowns(unknowns: readonly UnknownField[]): string {
  ...
  const lines = unknowns.map(
    (u) => `- ${u.field}: UNKNOWN (${u.reason})${u.detail ? ` â€” ${u.detail}` : ""}`,
  );
 ...
 * pure function of the record array â€” same input, same window, every time.
 *
...
export function buildGroundedUserPrompt(req: ExplanationRequest): string {
  ...
  let evidenceBlock =
    bundle.records.length === 0
      ? "(no evidence records â€” every on-chain question must be answered UNKNOWN)"
  ...
  const windowNote =
    omittedBulk > 0
      ? [
          ...
          "from this context for size. Everything not listed above is UNKNOWN â€”",
